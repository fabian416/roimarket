        ...
        using (MySqlCommand command = new MySqlCommand("SELECT * FROM products WHERE product_id = @product_id", connection))
        {
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                if (reader.Read())
                {
                    ...
                    {
                        productId = reader.GetInt32("product_id"), // Corregido aquí
                        productName = reader.GetString("product_name"), // Corregido aquí
                        price = (float)reader.GetDouble("price"),
                        stock = reader.GetInt32("stock"),
                        categoryIndex = reader.GetInt32("category_index") // Corregido aquí
                    };
                }
            }
        }
